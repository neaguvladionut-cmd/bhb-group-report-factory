...
2. Pasul 1 — **Ingestie**: completează numele proiectului și adaugă exportul AC de sinteză (scoruri 1–5) și, pentru comportamente, exportul AC detaliat (scoruri 0–2).
3. Pasul 2 — **Verificare**: rezolvă blocajele semnalate și marchează avertismentele după ce le-ai revizuit. Zero observat rămâne zero; o celulă goală este avertisment, nu este transformată în zero. Participanții complet neevaluati rămân vizibili în audit, dar sunt excluși din calcule.
4. Pasul 3 — **Structură**: verifică numărul de slide-uri, apoi alege rendererul **BHB** sau **TREND** și livrarea într-un singur PPTX sau în două fișiere separate (raport principal + anexă).
5. Pasul 4 — **Generare**: descarcă auditul XLSX și livrabilul/livrabilele PowerPoint. Completează interpretarea, recomandările și excepțiile în debrief-ul consultantului.
